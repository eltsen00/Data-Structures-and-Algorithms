...
/*对单调栈的理解：
    ...
    单调栈的核心思想是：通过“破坏”单调性来找到答案。
        为了维持栈的单调性，当一个新元素准备入栈时，我们需要进行一个特殊的检查：对于单调递增栈 (Next Smaller Element)，不断地将栈顶元素弹出，直到栈顶元素小于等于 x（或者栈为空）。然后，再将新元素 x 压入栈中。对于单调递减栈 (Next Greater Element)，不断地将栈顶元素弹出，直到栈顶元素大于等于 x（或者栈为空）。然后，再将新元素 x 压入栈中。
