...
vector<ULL> prefix_hash;   // 前缀哈希数组, prefix_hash[i] 存储 s[0..i-1] 的哈希值(前缀和技巧)
...
inline ULL get_hash(int left, int right) {
    return prefix_hash[right + 1] - prefix_hash[left] * pow_base[right - left + 1]; // 计算 s[left..right] 的哈希值，right - left + 1 是子串长度
}
...
int findTwinSubstring(const string& s) {
    ...
    int result = 0;
    while(left <= right) {
        int subLen = left + (right - left) / 2;
        if(check(s, subLen)) {
            result = subLen;    // 记录当前找到的最大长度
            left = subLen + 1;  // 尝试寻找更长的重复子串
        }
        else {
            right = subLen - 1;  // 搜索未果尝试寻找更短的重复子串
        }
    }
    return result;
}
